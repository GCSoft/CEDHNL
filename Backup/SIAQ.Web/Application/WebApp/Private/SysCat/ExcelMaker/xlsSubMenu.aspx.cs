...
'
' Descripción:
'           Crear el reporte de excel de SubMenu
'----------------------------------------------------------------------------------------------------------------------------------*/
...
{
   public partial class xlsSubMenu : System.Web.UI.Page
   {
      ...
      // Utilerías
		...
		Boolean HasValidParams(String sKey, ref ENTSubMenu oENTSubMenu){
            ...
            // Parámetros de consulta (idMenu|sNombre|tiActivo)
            oENTSubMenu.idMenu = Int32.Parse(sKey.Split(new Char[] { '|' })[0].ToString());
            oENTSubMenu.sNombre = sKey.Split(new Char[] { '|' })[1].ToString();
            oENTSubMenu.tiActivo = Int16.Parse(sKey.Split(new Char[] { '|' })[2].ToString());
		...
		void ExportExcel(DataTable oDataTable, String sSheetName){
            ...
            sGeneralTitle = "SIAQ - SubMenús al " + DateTime.Now.ToString("d");
		...
		protected void Page_Load(object sender, EventArgs e){
			...
			try
			{
				...
				if (!HasValidParams(this.Request.QueryString["key"].ToString(), ref oENTSubMenu)){
					this.Response.Redirect("~/Application/WebApp/Private/SysApp/saNotificacion.aspx?key=" + sDefaultErrorMessage, false);
				}
				
				// Consultar Órdenes de Compra
				SelectSubMenu(oENTSubMenu);
