    ...
    public partial class opeBusquedaSolicitud : BPPage
    {
		...
		void RecoveryForm(){
            ...
            try
            {
				...
				dtBeginDate = oBPSolicitud.SolicitudEntity.FechaDesde;
				dtEndDate = oBPSolicitud.SolicitudEntity.FechaHasta;
		...
		void SelectSolicitud( Boolean Recovery){
			...
			try
            {
				...
				BPSolicitud.SolicitudEntity.FormaContactoId = Int32.Parse(this.ddlFormaContacto.SelectedValue);
				BPSolicitud.SolicitudEntity.FuncionarioId = Int32.Parse(this.ddlFuncionario.SelectedValue);
				BPSolicitud.SolicitudEntity.FechaDesde =  (Recovery ? dtBeginDate : DateTime.Parse(this.wucBeginDate.BeginDate));
				BPSolicitud.SolicitudEntity.FechaHasta = (Recovery ? dtEndDate : DateTime.Parse(this.wucEndDate.EndDate));

                // Transacción
				BPSolicitud.SelectSolicitud();

                // Validaciones
				if (BPSolicitud.ErrorId != 0) { throw (new Exception(BPSolicitud.ErrorDescription)); }

                // Llenado de grid
				if (BPSolicitud.SolicitudEntity.ResultData.Tables[0].Rows.Count > 0){

					this.gvSolicitud.DataSource = BPSolicitud.SolicitudEntity.ResultData;
					this.gvSolicitud.DataBind();  
                }
