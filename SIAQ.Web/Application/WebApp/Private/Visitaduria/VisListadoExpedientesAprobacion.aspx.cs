	...
	public partial class VisListadoExpedientesAprobacion : BPPage
	{
		...
		protected void Page_Load(object sender, EventArgs e){
			try
			{
				...
				ScriptManager.RegisterStartupScript(this.Page, this.GetType(), Convert.ToString(Guid.NewGuid()), "tinyboxMessage('" + gcJavascript.ClearText(ex.Message) + "', 'Fail', true); focusControl('" + this.ddlArea.ClientID + "'); ", true);
			}
		}
		...
		protected void gvExpediente_RowCommand(object sender, GridViewCommandEventArgs e){
			...
			try
			{
				...
				switch (strCommand){
					case "Editar":
						this.Response.Redirect("VisDetalleExpedienteCierre.aspx?key=" + ExpedienteId + "|1", false);
						break;
				}
