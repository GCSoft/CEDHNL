...
namespace SIAQ.Web.Include.WebUserControls
{
    public partial class wucBusquedaUsuario : System.Web.UI.UserControl
    {
      ...
      protected void gvUsuario_Sorting(object sender, GridViewSortEventArgs e){
         ...
         DataView viewUsuario = null;

         try
         {

            // Obtener DataTable y DataView del GridView
            tblUsuario = utilFunction.ParseGridViewToDataTable(this.gvUsuario, true);
            viewUsuario = new DataView(tblUsuario);

            // Determinar ordenamiento
            this.hddSort.Value = (this.hddSort.Value == e.SortExpression ? e.SortExpression + " DESC" : e.SortExpression);

            // Ordenar vista
            viewUsuario.Sort = this.hddSort.Value;

            // Vaciar datos
            this.gvUsuario.DataSource = viewUsuario;
            this.gvUsuario.DataBind();
